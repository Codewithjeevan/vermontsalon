...
class Room extends Cl_Controller {
    ...
    public function __construct() {
        ...
        $function = "";
        $controller = "358";
        if($segment_2=="addEditRoom"){
            $function = "add";
        }elseif($segment_2=="addEditRoom" && $segment_3){
            $function = "edit";
        }elseif($segment_2=="deleteRoom"){
            $function = "delete";
        }elseif($segment_2=="roomList"){
            $function = "list";
        }else{
        ...
        if (htmlspecialcharscustom($this->input->post('submit'))) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if ($id == "") {
                    ...
                    $this->Common_model->insertInformation($fmc_info, "tbl_rooms");
                    $this->session->set_flashdata('exception', lang('insertion_success'));
                } else {
                    $this->Common_model->updateInformation($fmc_info, $id, "tbl_rooms");
                    $this->session->set_flashdata('exception', lang('update_success'));
                }
                if($add_more == 'add_more'){
                    redirect('Room/addEditRoom');
                }else{
                ...
                if ($id == "") {
                    ...
                    $data['category_information'] = $this->Common_model->getDataById($id, "tbl_rooms");
    ...
    public function roomList() {
        ...
        $data = array();
        $data['rooms'] = $this->Common_model->getAllByCompanyIdWithAddedBy ($company_id, "tbl_rooms");
        $data['main_content'] = $this->load->view('master/room/roomList', $data, TRUE);
        $this->load->view('userHome', $data);
    }
