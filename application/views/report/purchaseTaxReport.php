...
<h3 class="display_none"></h3>
    ...
    <section class="content-header">
        <div class="row justify-content-between">
            <div class="col-6 p-0">
                <h3 class="top-left-header mt-2"><?php echo lang('purchase_tax_report'); ?></h3>
            </div>
            <?php $this->view('updater/breadcrumb', ['firstSection'=> lang('report'), 'secondSection'=> lang('purchase_tax_report')])?>
        </div>
    </section>
    ...
    <div class="box-wrapper">
        ...
        <div class="report_header">
            ...
            <h5 class="outlet_info">
                <strong><?php echo lang('purchase_tax_report'); ?></strong>
            </h5>
        ...
        <div class="table-box">
            <div class="box-body">
                ...
                <input type="hidden" class="datatable_name"  data-filter="yes" data-title="<?php echo lang('purchase_tax_report'); ?>" data-id_name="datatable">
                    <table id="datatable" class="table">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center"><?php echo lang('vat'); ?> %</th>
                                <th class="text-center"><?php echo lang('taxable_amount'); ?></th>
                                <th class="text-center"><?php echo lang('grand_total'); ?></th>
                        ...
                        <tbody>
                            ...
                            $vatTotal = 0;
                            $taxableTotal = 0;
                            if (isset($purchaseTaxReport)):
                                foreach ($purchaseTaxReport as $key => $value) {
                                    if ((float)$value->vat_amount > 0):
                                        $grandTotal  += (float)$value->grand_total;
                                        $vatTotal    += (float)$value->vat_amount;
                                        $taxableTotal += (float)$value->taxable_amount;
                                        $key++;
                            ?>
                            <tr>
                                <td><?php echo escape_output($key); ?></td>
                                <td><?php echo escape_output($value->reference_no); ?></td>
                                <td><?php echo escape_output($value->invoice_no); ?></td>
                                <td><?php echo dateFormat($value->date); ?></td>
                                <td><?php echo escape_output($value->supplier_name); ?></td>
                                <td class="text-center"><?php echo number_format((float)$value->vat_percentage, 2); ?>%</td>
                                <td class="text-center"><?php echo getAmtCustom($value->taxable_amount); ?></td>
                                <td class="text-center"><?php echo getAmtCustom($value->grand_total); ?></td>
                                <td class="text-right"><?php echo getAmtCustom($value->vat_amount); ?></td>
                            </tr>
                            <?php
                                    endif;
                                }
                            ...
                            <tr>
                                ...
                                <th></th>
                                <th class="text-center"><?php echo getAmtCustom($taxableTotal); ?></th>
                                <th class="text-center"><?php echo getAmtCustom($grandTotal); ?></th>
                                <th class="text-right"><?php echo getAmtCustom($vatTotal); ?></th>
                            </tr>
                        </tbody>
                    </table>
                </div>
...
<div id="product-filter" class="filter-modal">
    <div class="filter-modal-body">
        ...
        <?php echo form_open(base_url() . 'Report/purchaseTaxReport') ?>
        <div class="row">
            ...
            <div class="col-sm-12 col-md-6 mb-2">
                <div class="form-group">
                    <select class="form-control select2" id="supplier_id" name="supplier_id">
                        ...
                        ?>
                            <option <?= set_select('supplier_id',$value->id)?> value="<?php echo escape_output($value->id) ?>"><?php echo escape_output($value->name) ?></option>
                        <?php
                            endforeach;
                        }
                        ?>
